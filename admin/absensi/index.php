...
if ($gid && $tgl && $kid) {
    $info = mysqli_fetch_assoc(mysqli_query($koneksi,
        "SELECT g.nama AS nama_guru, k.nama_kelas, mp.nama_mapel
         FROM guru g
         JOIN jadwal j ON j.guru_id = g.id AND j.kelas_id = '$kid'
    ...
    $nama_mapel = $info ? $info['nama_mapel'] : '-';
?>
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
            <span>
                ...
                <span class="badge bg-success ms-1">Guru: <?= htmlspecialchars($nama_guru) ?></span>
...
if ($gid) {
    ...
    $data = mysqli_query($koneksi,
        "SELECT a.tanggal, a.kelas_id, k.nama_kelas,
         ...
         WHERE EXISTS (SELECT 1 FROM jadwal j
                       WHERE j.kelas_id = a.kelas_id
                         AND j.guru_id = '$gid'
                         AND (a.mapel_id IS NULL OR a.mapel_id = j.mapel_id))
...
<div class="main-content">
    ...
    <div class="page-header d-flex justify-content-between align-items-center flex-wrap gap-2">
        <h4 class="mb-0"><i class="fas fa-calendar-week text-gold me-2"></i>Absensi Guru</h4>
    ...
    <div class="alert alert-info d-flex align-items-center gap-2">
        ...
        <div>
            <strong><?= htmlspecialchars($guru['nama'] ?? $guru['nama_lengkap']) ?></strong><br>
            ...
            <div class="table-responsive">
                <table class="table table-hover table-bordered align-middle mb-0">
                    ...
                    <tr>
                        ...
                        <td><span class="badge bg-secondary"><?= htmlspecialchars($r['nama_kelas']) ?></span></td>
                        <td><span class="badge bg-info"><?= htmlspecialchars($r['nama_mapel'] ?? '-') ?></span></td>
...
$guru_list = mysqli_query($koneksi,
    "SELECT g.id, g.nama, g.nip,
     ...
     JOIN absensi a ON a.kelas_id = j.kelas_id
                   AND (a.mapel_id IS NULL OR a.mapel_id = j.mapel_id)
     GROUP BY g.id
     ORDER BY g.nama");
?>
...
<div class="main-content">
    ...
    <?php if (isset($_GET['success'])): ?>
    <div class="alert alert-success alert-auto"><i class="fas fa-check-circle"></i> <?= htmlspecialchars($_GET['success']) ?></div>
    <?php endif; ?>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover table-bordered align-middle mb-0">
                    ...
                    <tr>
                        <td><?= $no++ ?></td>
                        <td><i class="fas fa-user-tie text-primary me-1"></i> <strong><?= htmlspecialchars($g['nama'] ?? '') ?></strong></td>
